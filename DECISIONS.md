...
referenced paths must be safe relative paths that cannot escape the supplied validation root; and a declared
derivative must carry transformation history. The validator is read-only, is deterministic, exits nonzero for
every rejection, and never rewrites a manifest or moves/promotes material. This decision only formalizes the
enforcement *mechanism* — it does not loosen D1, D19, or D21's substantive data-boundary rules, and a passing
validator result does not by itself authorize the bounded pilot (section E) or any real dataset; see
`docs/promotion-controls.md` (issue #18).
